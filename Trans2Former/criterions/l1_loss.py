...
class PretrainL1Loss(FairseqCriterion):
    """
    Implementation for the L1 loss (MAE loss) used in edgeformer model pretraining.
    """
    ...
    def forward(self, model, sample, reduce=True):
        ...
        3) logging output to display while training
        """
        sample_size = sample['nsamples']

        logits = model(**sample['net_input'])
        logits = logits[:, 0, :].squeeze(1)
        targets = model.get_targets(sample, [logits]).type(logits.dtype)
...
class PretrainL1LossWithFlag(FairseqCriterion):
    """
    Implementation for the L1 loss (MAE loss) used in edgeformer model pretraining.
    """
